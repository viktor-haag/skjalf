...
from skjalf.watcher.face_search import FaceSearcher
...
class TestFaceSearcherFuzzyMatch(unittest.TestCase):
    ...
    def test_exact_match(self):
        ...
        searcher = FaceSearcher(names_path=self.names_path)
...
class TestFaceSearcherSearch(unittest.TestCase):
    ...
    @patch("skjalf.watcher.face_search.chromadb.PersistentClient")
    def test_match_with_no_chromadb_results(
        self, mock_client, mock_embed
    ):
        ...
        searcher = FaceSearcher(names_path=self.names_path)
        # Set up a folder DB dir so the search actually tries to query
        db_dir = Path(self.temp_dir.name) / ".skjalf"
        db_dir.mkdir()
        searcher.set_folder_db_dirs([db_dir])
    ...
    def test_match_with_chromadb_results(
        ...
        mock_collection.query.return_value = {
            "ids": [[str(test_img)]],
            "metadatas": [[{"abs_path": str(test_img), "name": "Jacqueline"}]],
            "distances": [[0.1]],
        }
        mock_client.return_value.get_collection.return_value = mock_collection
...
class TestFaceSearcherNamesFile(unittest.TestCase):
    ...
    def test_empty_names_file(self):
        ...
        try:
            searcher = FaceSearcher(names_path=temp_path)
            self.assertEqual(searcher._names, {})
        finally:
            temp_path.unlink()


class TestFaceSearcherSetFolderDbDirs(unittest.TestCase):
    """Test that set_folder_db_dirs correctly configures search scope."""
    ...
    def test_default_db_dirs_empty(self):
        """By default, no folder DB dirs are set."""
        searcher = FaceSearcher(names_path=self.names_path)
        self.assertEqual(searcher._face_db_dirs, [])

    def test_set_folder_db_dirs(self):
        """set_folder_db_dirs should store the list of db dirs."""
        searcher = FaceSearcher(names_path=self.names_path)
        db_dirs = [
            Path("/data/photos1/.skjalf"),
            Path("/data/photos2/.skjalf"),
        ]
        searcher.set_folder_db_dirs(db_dirs)
        self.assertEqual(searcher._face_db_dirs, db_dirs)
